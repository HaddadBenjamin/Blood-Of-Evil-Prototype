		...
		protected virtual void	Awake()
		{
			...
			foreach (var assembly in assemblies)
			{
				if (assembly.FullName.StartsWith("Assembly-CSharp-Editor") == true)
				{
					...
					this.NGConsoleType = this.editorAssembly.GetType("NGToolsEditor.NGConsoleWindow");
					...
					this.ModuleType = this.editorAssembly.GetType("NGToolsEditor.Module");
					this.IdFieldInfo = this.ModuleType.GetField("id", BindingFlags.Instance | BindingFlags.NonPublic | BindingFlags.Public);

					this.MainModuleType = this.editorAssembly.GetType("NGToolsEditor.MainModule");

					this.RecorderModuleType = this.editorAssembly.GetType("NGToolsEditor.RecorderModule");
		...
		protected virtual void	OnGUI()
		{
			...
			if (instances.Length == 0)
			{
				if (GUI.Button(r, "Open NG Console before starting the tutorial") == true)
					EditorApplication.ExecuteMenuItem("Window/NG Console");
				return;
			}
