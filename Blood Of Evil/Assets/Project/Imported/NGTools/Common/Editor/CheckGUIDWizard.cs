...
namespace NGToolsEditor
{
	...
	[InitializeOnLoad]
	public class CheckGUIDWizard : ScriptableWizard
	{
		[Serializable]
		public class Entry
		{
		...
		static	CheckGUIDWizard()
		{
			Utility.AddMenuItemPicker(Constants.MenuItemPath + CheckGUIDWizard.Title);
		}

		[MenuItem(Constants.MenuItemPath + CheckGUIDWizard.Title, priority = Constants.MenuItemPriority + 369)]
		private static void	Open()
		{
			ScriptableWizard.GetWindow<CheckGUIDWizard>(true, CheckGUIDWizard.Title, true);
		}
				...
				using (LabelWidthRestorer.Get(50F))
				{
					for (int i = 0; i < this.GUIDs.Count; i++)
					{
						EditorGUILayout.BeginHorizontal();
						{
							...
							{
								Rect r = GUILayoutUtility.GetRect(0F, 0F, GUILayout.Width(32F), GUILayout.Height(32F));
								EditorGUI.DrawTextureTransparent(r, this.entries[i].assetPreview, ScaleMode.ScaleToFit);
							}
						}
		...
		{
			if (EditorGUIUtility.systemCopyBuffer.Length == 32 && this.lastCopyBuffer != EditorGUIUtility.systemCopyBuffer)
			{
