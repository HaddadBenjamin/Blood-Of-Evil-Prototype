...
{
	[InitializeOnLoad]
	public class NGInspectorWindow : NGRemoteWindow, IHasCustomMenu
	{
		...
		public static Color	MaterialHeaderBackgroundColor;
		...
		static	NGInspectorWindow()
		{
			Utility.AddMenuItemPicker(Constants.MenuItemPath + NGInspectorWindow.NormalTitle);

			if (EditorGUIUtility.isProSkin == true)
				NGInspectorWindow.MaterialHeaderBackgroundColor = new Color(62F / 255F, 62F / 255F, 62F / 255F, 1F);
			else
				NGInspectorWindow.MaterialHeaderBackgroundColor = new Color(162F / 255F, 162F / 255F, 162F / 255F, 1F);
		}

		[MenuItem(Constants.MenuItemPath + NGInspectorWindow.NormalTitle, priority = Constants.MenuItemPriority + 215)]
		private static void	Open()
		{
			EditorWindow.GetWindow<NGInspectorWindow>(NGInspectorWindow.ShortTitle);
		}

		protected virtual void	OnEnable()
		{
			this.SetTitle(NGInspectorWindow.ShortTitle);
			...
			Utility.RegisterIntervalCallback(this.Repaint, NGInspectorWindow.ForceRepaintRefreshTick);
			...
			EditorGUILayout.BeginHorizontal(GeneralStyles.Toolbar);
			{
				...
				if (this.Hierarchy.Client != null)
				{
					...
					if (EditorGUI.EndChangeCheck() == true)
					{
						...
						{
							if (EditorUtility.DisplayDialog(LC.G("NGInspector_Batch"), LC.G("NGHierarchy_RequesttoSendCurrentBatch"), LC.G("Yes"), LC.G("No")) == true)
		...
		protected override void	OnGUIConnected()
		{
			...
			EditorGUILayout.BeginHorizontal();
			{
				...
				using (this.animName.Restorer(0F, .8F + this.animName.Value, 0F, 1F))
				{
					...
					if (EditorGUI.EndChangeCheck() == true)
						this.Hierarchy.Client.AddPacket(new ClientUpdateFieldValuePacket(this.target.instanceID + ".name", this.stringHandler.Serialize(name), this.stringHandler));
				}
				...
				using (this.animIsStatic.Restorer(0F, .8F + this.animIsStatic.Value, 0F, 1F))
				{
					...
					if (EditorGUI.EndChangeCheck() == true)
						this.Hierarchy.Client.AddPacket(new ClientUpdateFieldValuePacket(this.target.instanceID + ".isStatic", this.booleanHandler.Serialize(isStatic), this.booleanHandler));
				}
			}
			EditorGUILayout.EndHorizontal();
			...
			this.bodyRect.y = 52F + NGInspectorWindow.ComponentSpacing;
			this.bodyRect.width = this.position.width;
			this.bodyRect.height = this.position.height - 52F;
			...
			for (int i = 0; i < this.target.components.Count; i++)
			{
				try
				{
					this.viewRect.height += this.target.components[i].GetHeight(this) + NGInspectorWindow.ComponentSpacing;
				}
				catch
				{
					this.viewRect.height += 16F + NGInspectorWindow.ComponentSpacing;
				}
			}
			...
			this.scrollPosition = GUI.BeginScrollView(this.bodyRect, this.scrollPosition, this.viewRect);
			{
				...
				for (int i = 0; i < this.target.components.Count; i++)
				{
					try
					{
						...
						this.r.y += height + NGInspectorWindow.ComponentSpacing;
					...
					catch (Exception ex)
					{
						if (Event.current.type == EventType.Repaint)
							EditorGUI.DrawRect(this.r, Color.red);

						EditorGUI.LabelField(this.r, "Error", "Component " + this.target.components[i].name + " (" + i + ") failed." + ex.Message);
						this.r.y += 16F + NGInspectorWindow.ComponentSpacing;
						if (this.r.y - this.scrollPosition.y > this.bodyRect.height)
							break;
						//InternalNGDebug.LogException("Component " + this.target.components[i].name + " (" + i + ") failed.", ex);
					}
				}
		...
		private void	DrawMaterials(Rect r, List<int> materials)
		{
			...
			for (int i = 0; i < materials.Count; i++)
			{
				...
				if (material != null)
				{
					...
					if (Event.current.type == EventType.Repaint)
						EditorGUI.DrawRect(r, NGInspectorWindow.MaterialHeaderBackgroundColor);
					...
					if (shaderNames != null)
					{
						...
						if (GUI.Button(r, LC.G("Change")) == true)
							this.Hierarchy.Client.AddPacket(new ClientChangeMaterialShaderPacket(materials[i], shaderInstanceIDs[material.selectedShader]));
					...
					for (int j = 0; j < material.properties.Length; j++)
					{
						...
						if (material.properties[j].type == NGShader.ShaderPropertyType.Color)
						{
							...
							if (EditorGUI.EndChangeCheck() == true)
								this.Hierarchy.Client.AddPacket(new ClientUpdateMaterialPropertyPacket(materials[i], material.properties[j].name, this.colorHandler.Serialize(newValue)));
						}
						else if (material.properties[j].type == NGShader.ShaderPropertyType.Float)
						{
							...
							if (EditorGUI.EndChangeCheck() == true)
								this.Hierarchy.Client.AddPacket(new ClientUpdateMaterialPropertyPacket(materials[i], material.properties[j].name, this.floatHandler.Serialize(newValue)));
						}
						...
						else if (material.properties[j].type == NGShader.ShaderPropertyType.TexEnv)
						{
							...
							if (EditorGUI.EndChangeCheck() == true)
								this.Hierarchy.Client.AddPacket(new ClientUpdateMaterialVector2Packet(materials[i], material.properties[j].name, newValue, ClientUpdateMaterialVector2Packet.Type.Scale));

							r.y += r.height;
							EditorGUI.BeginChangeCheck();
							newValue = EditorGUI.Vector2Field(r, "Offset", material.properties[j].textureOffset);
							if (EditorGUI.EndChangeCheck() == true)
								this.Hierarchy.Client.AddPacket(new ClientUpdateMaterialVector2Packet(materials[i], material.properties[j].name, newValue, ClientUpdateMaterialVector2Packet.Type.Offset));
							--EditorGUI.indentLevel;
		...
		private void	DrawVector4(Rect r, int materialInstanceID, NetMaterialProperty property)
		{
			...
			using (LabelWidthRestorer.Get(14F))
			{
				...
				if (EditorGUI.EndChangeCheck() == true)
				{
					...
					this.Hierarchy.Client.AddPacket(new ClientUpdateMaterialPropertyPacket(materialInstanceID, property.name, this.vector4Handler.Serialize(property.vectorValue)));
				}
				...
				EditorGUILayout.BeginHorizontal();
				{
					...
					using (this.animTag.Restorer(0F, .8F + this.animTag.Value, 0F, 1F))
					{
						using (LabelWidthRestorer.Get(30F))
						{
							...
							{
								EditorGUI.BeginChangeCheck();
								string	tag = EditorGUILayout.TagField("Tag", this.target.tag);
								if (EditorGUI.EndChangeCheck() == true)
								{
									this.Hierarchy.Client.AddPacket(new ClientUpdateFieldValuePacket(this.target.instanceID + ".tag", this.stringHandler.Serialize(tag), this.stringHandler));
								}
							}
							else
							{
								EditorGUI.BeginChangeCheck();
								string	tag = EditorGUILayout.TextField("Tag", this.target.tag);
								if (EditorGUI.EndChangeCheck() == true)
								{
									this.Hierarchy.Client.AddPacket(new ClientUpdateFieldValuePacket(this.target.instanceID + ".tag", this.stringHandler.Serialize(tag), this.stringHandler));
								}
					...
					using (this.animLayer.Restorer(0F, .8F + this.animLayer.Value, 0F, 1F))
					{
						using (LabelWidthRestorer.Get(50F))
						{
							...
							{
								EditorGUI.BeginChangeCheck();
								int	layer = EditorGUILayout.Popup("Layer", this.target.layer, this.Hierarchy.Layers);
								if (EditorGUI.EndChangeCheck() == true)
								{
									this.Hierarchy.Client.AddPacket(new ClientUpdateFieldValuePacket(this.target.instanceID + ".layer", this.intHandler.Serialize(layer), this.intHandler));
								}
							}
						}
					}
		...
		{
			this.bodyRect = new Rect(0F, EditorGUIUtility.singleLineHeight + NGInspectorWindow.ComponentSpacing, this.position.width, this.position.height - EditorGUIUtility.singleLineHeight);
		...
		void	IHasCustomMenu.AddItemsToMenu(GenericMenu menu)
		{
			...
			menu.AddSeparator("");
			Utility.AddNGMenuItems(menu, this, NGInspectorWindow.NormalTitle, Constants.WikiBaseURL + "#markdown-header-132-ng-remote-inspector");
		}
	}
}