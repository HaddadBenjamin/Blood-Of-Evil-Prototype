...
namespace NGToolsEditor
{
	...
	[InitializeOnLoad]
	public class NGHierarchyWindow : EditorWindow, IHierarchyManagement, IHasCustomMenu, IUnityData
	{
		public class NGServerInstance
		{
		...
		public const double	UDPServerPingLifetime = NGServerScene.UDPPingInterval * 3D;
		...
		private Dictionary<NGInspectorWindow, int>		watchersGameObject = new Dictionary<NGInspectorWindow, int>();
		private Dictionary<NGInspectorWindow, int[]>	watchersMaterials = new Dictionary<NGInspectorWindow, int[]>();
		...
		#region UDP Servers
		private List<NGServerInstance>	NGServerInstances;
		...
		static	NGHierarchyWindow()
		{
			Utility.AddMenuItemPicker(Constants.MenuItemPath + NGHierarchyWindow.NormalTitle);
		}

		[MenuItem(Constants.MenuItemPath + NGHierarchyWindow.NormalTitle, priority = Constants.MenuItemPriority + 210)]
		private static void	Open()
		{
			EditorWindow.GetWindow<NGHierarchyWindow>(NGHierarchyWindow.ShortTitle);
		}
		...
		protected virtual void	OnEnable()
		{
			...
			this.NGServerInstances = new List<NGServerInstance>();

			this.StartUDPBroadcastServer();
		...
		protected virtual void	OnGUI()
		{
			EditorGUILayout.BeginHorizontal("Toolbar");
			{
				...
				{
					EditorGUI.BeginDisabledGroup(this.NGServerInstances.Count == 0);
					{
						if (this.NGServerInstances.Count == 0)
							Utility.content.text = "No server";
						else if (this.NGServerInstances.Count == 1)
							Utility.content.text = "1 server";
						else
							Utility.content.text = this.NGServerInstances.Count + " servers";
						...
						if (GUI.Button(r, Utility.content, GeneralStyles.ToolbarDropDown) == true)
						{
							...
							for (int i = 0; i < this.NGServerInstances.Count; i++)
							{
								...
								if (isConnected == true && this.NGServerInstances[i].server == this.address + ":" + this.port)
									current = true;

								menu.AddItem(new GUIContent(this.NGServerInstances[i].server), current, this.OverrideAddressPort, this.NGServerInstances[i].server);
							}
				...
				{
					EditorGUI.BeginDisabledGroup(Utility.GetAsyncProgressBarInfo() == NGHierarchyWindow.ProgressBarConnectingString);
					{
			...
			if (this.showOptions == true)
			{
				...
				EditorGUILayout.BeginHorizontal("Toolbar");
				{
					...
					this.networkRefresh = EditorGUILayout.FloatField(Utility.content, this.networkRefresh);
				...
				EditorGUILayout.BeginHorizontal("Toolbar");
				{
					...
					this.syncTags = GUILayout.Toggle(this.syncTags, Utility.content, GeneralStyles.ToolbarToggle);
					Utility.content.tooltip = string.Empty;
			...
			{
				EditorGUI.BeginChangeCheck();
				this.searchPattern = EditorGUILayout.TextField(this.searchPattern, GeneralStyles.ToolbarSeachTextField);
				if (EditorGUI.EndChangeCheck() == true)
				...
				this.root.children.Count > 0)
			{
				...
				this.bodyRect.x = 0F;
				this.bodyRect.y = this.showOptions == true ? 94F : 40F;
				...
				if (string.IsNullOrEmpty(this.searchPattern) == false)
				{
					...
					if (last != null)
					{
						...
						if (Event.current.type == EventType.Repaint)
						{
							...
							EditorGUI.DrawRect(bodyRect, new Color(60F / 255F, 60F / 255F, 60F/ 255F, 1F));
		...
		protected virtual void	Update()
		{
			...
			if (this.IsClientConnected() == true)
			{
				...
				else if (this.ping.isDone == true)
				{
					...
					this.ping = null;
					this.nextPingTime = Time.realtimeSinceStartup + NGHierarchyWindow.PingInterval;
					this.Repaint();
				}
			}
		...
		{
			Utility.StartAsyncBackgroundCallback(this.OpenClient, NGHierarchyWindow.ProgressBarConnectingString, 10F, "Connecting to server is too long and was aborted.");
		}
		...
		private void	OpenClient()
		{
			try
			{
				...
				lock (this.NGServerInstances)
				{
					if (this.NGServerInstances.Exists(s => s.server == server) == false)
						this.NGServerInstances.Add(new NGServerInstance() { server = server, pingMaxLastTime = Utility.ConvertToUnixTimestamp(DateTime.Now) + NGHierarchyWindow.UDPServerPingLifetime });
				}
			}
			catch (Exception ex)
			{
		...
		public void		SelectGameObject(int instanceID)
		{
			...
			if (n != null)
			{
				...
				NGInspectorWindow[]	inspectors = EditorWindow.FindObjectsOfType<NGInspectorWindow>();
		...
		public void		WatchGameObject(NGInspectorWindow inspector, ClientGameObject gameObject)
		{
			if (gameObject == null)
			{
		...
		public void		WatchMaterials(NGInspectorWindow inspector, List<int> materialIDs)
		{
				...
				if (int.TryParse(paths[1], out instanceID) == false)
				{
					...
					switch (paths[1])
					{
						...
						case "name":
							...
							NGHierarchyWindow.Buffer.Clear();
							NGHierarchyWindow.Buffer.Append(rawValue);

							if (paths[1] == "tag")
								go.tag = typeHandler.Deserialize(NGHierarchyWindow.Buffer, typeof(string)) as string;
							else if (paths[1] == "name")
								go.name = typeHandler.Deserialize(NGHierarchyWindow.Buffer, typeof(string)) as string;
							break;
						...
						case "isStatic":
							...
							if (paths[1] == "active")
								go.active = (bool)typeHandler.Deserialize(NGHierarchyWindow.Buffer, typeof(bool));
							else if (paths[1] == "isStatic")
								go.isStatic = (bool)typeHandler.Deserialize(NGHierarchyWindow.Buffer, typeof(bool));
							break;
						...
						case "layer":
							...
							go.layer = (int)typeHandler.Deserialize(NGHierarchyWindow.Buffer, typeof(int));
							break;
		...
		/// <param name="type"></param>
		/// <exception cref="UnityEngine.MissingFieldException">Thrown when the property was not found in the material.</exception>
		public void		UpdateMaterialVector2(int instanceID, string propertyName, Vector2 value, ClientUpdateMaterialVector2Packet.Type type)
		{
			NetMaterial	material;
			...
			if (this.materials.TryGetValue(instanceID, out material) == true)
			{
				for (int i = 0; i < material.properties.Length; i++)
				{
					if (material.properties[i].name.Equals(propertyName) == true)
					{
						if (type == ClientUpdateMaterialVector2Packet.Type.Offset)
							material.properties[i].textureOffset = value;
						else if (type == ClientUpdateMaterialVector2Packet.Type.Scale)
		...
		private Rect	DrawGameObject(Rect r, ref int p, ClientGameObject gameObject, bool hideChildren = false)
		{
			...
			if (gameObject.Selected == true)
			{
				...
				r.x = 0F;
				r.width += originX;
				EditorGUI.DrawRect(r, new Color(50F / 255F, 76F / 255F, 120F / 255F));
				r.width -= originX;
				r.x = originX;
			}
					...
					DragAndDrop.GetGenericData("p") is int)
				{
							 ...
							 DragAndDrop.visualMode == DragAndDropVisualMode.Move)
					{
						...
						if (Event.current.mousePosition.y - r.y <= 3F)
						{
							r2.height = 2F;
							r2.y -= 1F;
							EditorGUI.DrawRect(r2, NGHierarchyWindow.DropBackgroundColor);
						}
						// Bottom
						else if (Event.current.mousePosition.y - r.y >= 13F)
						{
			...
			if (i == this.paths.Length - 1)
			{
				...
				if (type.IsUnityArray() == true)
				{
					...
					if (array.array == null)
					{
						...
						ArrayData	newArray = typeHandler.Deserialize(NGHierarchyWindow.Buffer, array.type) as ArrayData;
					...
					typeHandler = TypeHandlersManager.GetTypeHandler(typeof(int));
					int	newSize = (int)typeHandler.Deserialize(NGHierarchyWindow.Buffer, typeof(int));
				...
				else
				{
					...
					object	v = typeHandler.Deserialize(NGHierarchyWindow.Buffer, type);
		...
		private object	ResolveArray(Type type, object instance, int i)
		{
			if (i == this.paths.Length - 1)
			{
				...
				return typeHandler.Deserialize(NGHierarchyWindow.Buffer, type);
			}
		...
		private void	StartUDPBroadcastServer()
		{
			if (this.UDPBroadcastServer != null && this.UDPBroadcastServer.Client.Connected == true)
				this.UDPBroadcastServer.Close();

			int	port = NGServerScene.UDPPortBroadcastMin;

			for (int i = NGServerScene.UDPPortBroadcastMin; i <= NGServerScene.UDPPortBroadcastMax; i++)
			{
				try
				{
					this.UDPBroadcastServer = new UdpClient(port);
					this.UDPBroadcastServer.BeginReceive(new AsyncCallback(this.OnUDPBroadcastPacketReceived), null);
		...
		{
			//if (this.UDPBroadcastServer.Client.Connected == false)
			//{
			//	InternalNGDebug.LogWarning("UDP Broadcast server not connected.");
			//	this.StartUDPBroadcastServer();
			//}
			...
			lock (this.NGServerInstances)
			{
				...
				for (int i = 0; i < this.NGServerInstances.Count; i++)
				{
					if (this.NGServerInstances[i].pingMaxLastTime + NGHierarchyWindow.UDPServerPingLifetime < now)
					{
		...
		private void	OnUDPBroadcastPacketReceived(IAsyncResult ar)
		{
			...
			lock (this.NGServerInstances)
			{
				...
				if (n == -1 && this.IsEqual(data, NGServerScene.UDPPingMessage) == true)
				{
					//InternalNGDebug.InternalLog("Add " + content + " (" + BitConverter.ToString(data) + ").");
					this.NGServerInstances.Add(new NGServerInstance() { server = content, pingMaxLastTime = Utility.ConvertToUnixTimestamp(DateTime.Now) + NGHierarchyWindow.UDPServerPingLifetime });
					this.forceRepaint = true;
				}
				else
				{
					if (this.IsEqual(data, NGServerScene.UDPEndMessage) == true)
					{
						//InternalNGDebug.InternalLog("Kill " + content + " (" + BitConverter.ToString(data) + ").");
						this.NGServerInstances.RemoveAt(n);
						this.forceRepaint = true;
					}
					else if (this.IsEqual(data, NGServerScene.UDPPingMessage) == true)
					{
						//InternalNGDebug.InternalLog("Alive " + content + " (" + BitConverter.ToString(data) + ").");
						this.NGServerInstances[n].pingMaxLastTime = Utility.ConvertToUnixTimestamp(DateTime.Now) + NGHierarchyWindow.UDPServerPingLifetime;
					}
					else
					{
		...
		/// <summary>
		/// Checks if a channel is free to use or not. Refer to NGHierarchyWindow.BlockRequestLifeTime for the lifetime.
		/// </summary>
		/// <param name="id"></param>
		/// <returns></returns>
		public bool	BlockRequestChannel(int id)
		{
			...
			this.blockedRequestChannels.Add(new KeyValuePair<int, float>(id, Time.realtimeSinceStartup + NGHierarchyWindow.BlockRequestLifeTime));
		...
		void	IHasCustomMenu.AddItemsToMenu(GenericMenu menu)
		{
			...
			menu.AddSeparator("");
			Utility.AddNGMenuItems(menu, this, NGHierarchyWindow.NormalTitle, Constants.WikiBaseURL + "#markdown-header-131-ng-remote-hierarchy");
#if NGT_DEBUG
			if (this.Client != null)
		...
		{
			menu.AddItem(new GUIContent("Add Tab/" + NGInspectorWindow.NormalTitle), false, new GenericMenu.MenuFunction(NGHierarchyWindow.AddTabInspector));
			menu.AddItem(new GUIContent("Add Tab/" + NGProjectWindow.NormalTitle), false, new GenericMenu.MenuFunction(NGHierarchyWindow.AddTabProject));
			menu.AddItem(new GUIContent("Add Tab/" + NGCameraWindow.NormalTitle), false, new GenericMenu.MenuFunction(NGHierarchyWindow.AddTabCamera));
			menu.AddSeparator("");
		}

		private static void	AddTabInspector()
		{
			NGInspectorWindow	inspector = EditorWindow.GetWindow<NGInspectorWindow>(NGInspectorWindow.ShortTitle, false, typeof(NGHierarchyWindow));
			inspector.SetHierarchy(EditorWindow.GetWindow<NGHierarchyWindow>(NGHierarchyWindow.ShortTitle));
			inspector.Show();
		}

		private static void	AddTabProject()
		{
			NGProjectWindow	project = EditorWindow.GetWindow<NGProjectWindow>(NGProjectWindow.ShortTitle, false, typeof(NGHierarchyWindow));
			project.SetHierarchy(EditorWindow.GetWindow<NGHierarchyWindow>(NGHierarchyWindow.ShortTitle));
			project.Show();
		}

		private static void	AddTabCamera()
		{
			NGCameraWindow	camera = EditorWindow.GetWindow<NGCameraWindow>(NGCameraWindow.ShortTitle, false, typeof(NGHierarchyWindow));
			camera.SetHierarchy(EditorWindow.GetWindow<NGHierarchyWindow>(NGHierarchyWindow.ShortTitle));
			camera.Show();
		}
	}
}