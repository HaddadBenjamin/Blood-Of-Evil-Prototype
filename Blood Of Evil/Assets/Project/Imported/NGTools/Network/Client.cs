...
namespace NGTools
{
	public class Client
	{
		...
		public void	ReadCallBack(IAsyncResult ar)
		{
			...
			{
				this.reader.BeginRead(this.tempBuffer, 0, this.tempBuffer.Length, new AsyncCallback(ReadCallBack), this);
		...
		public void	AddPacket(Packet command)
		{
			if (this.batchMode == BatchMode.On)
			{
				for (int i = 0; i < this.batchedPackets.Count; i++)
				{
					if (command.AggregateInto(this.batchedPackets[i]) == true)
						return;
				}

				this.batchedPackets.Add(command);
			}
			else
			{
				for (int i = 0; i < this.pendingPackets.Count; i++)
				{
					if (command.AggregateInto(this.pendingPackets[i]) == true)
						return;
				}

				this.pendingPackets.Add(command);
			}
		}
